...
function listNumbers(...numbers) {
  ...
  if (invalidParams.length > 0) {
    ...
    process.exitCode = 9; // Código de saída 9 para indicar erro de argumento inválido
  } else {
...
process.on('exit', (code) => {
  ...
  if (code === 9) {
    ...
    console.log('Processo encerrado devido a argumentação inválida em uma função', code);
  } else if (code === 0) {
    console.log('Processo encerrado normalmente', code);
  }
});
